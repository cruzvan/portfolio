...
const MenuCard: React.FC<MenuCardProps> = ({ 
  ...
  return (
    <div 
      ref={cardRef}
      className={`relative group cursor-pointer transition-all duration-500 ease-out ${flexClass} ${className} ${isHovered ? 'z-50' : 'z-10'} min-h-[140px] md:min-h-0`}
    ...
    >
      ...
      <div className={`w-full h-full opacity-0 animate-slide-up-fade-in overflow-hidden transition-shadow duration-300 ${isHovered ? 'shadow-[0_30px_60px_-10px_rgba(0,0,0,0.8)]' : 'shadow-2xl'}`} style={style}>
        ...
        <div className="absolute inset-0 w-full h-full bg-black">
          <div className={`absolute inset-0 z-30 pointer-events-none transition-all duration-300 border-[4px] ${
        ...
        <div className={`absolute bottom-0 left-0 w-full transition-all duration-300 flex flex-col justify-end z-20 ${
           isCompressed ? 'opacity-0 translate-y-10' : 'opacity-100 translate-y-0'
        }`}>
          ...
          >
              ...
              {/* Icon Container */}
              <div className={`h-10 w-10 md:h-14 md:w-14 flex-shrink-0 flex items-center justify-center transition-all duration-300 relative z-10 ${
                  isHovered ? 'bg-white text-[#FE4403] shadow-lg scale-110' : 'border border-white/20 bg-white/5 text-white/70'
              }`}>
              ...
              <div className="flex flex-col justify-center h-full relative z-10 pointer-events-none">
                  <h3 className={`text-[9px] md:text-sm uppercase tracking-widest transition-colors duration-300 font-bold mb-1 ${
                      isHovered ? 'text-black/60' : 'text-gray-400'
                  }`} style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                      {item.category}
                  </h3>
                  <h2 className={`font-bold uppercase leading-none tracking-tighter transition-all duration-300 ${
                      isHovered ? 'text-white text-lg md:text-2xl text-shadow-sm' : 'text-white/90 text-base md:text-xl'
                      }`}
...
const MainMenu: React.FC<MainMenuProps> = ({ onCardClick, onHeaderClick, score, onAddScore }) => {
  ...
  return (
    <div className="relative flex flex-col justify-center items-center w-full h-full animate-fade-in-fast">
      ...
      */}
      <div className="flex flex-col md:flex-row items-stretch justify-start md:justify-center z-10 w-full px-6 md:px-12 h-auto md:h-[70vh] overflow-y-auto md:overflow-visible mt-20 md:mt-0 pb-20 md:pb-0 gap-4 md:gap-0 scrollbar-hide">
        
        {columns.map((col, colIndex) => {
          const isColumnActive = hoveredColumnIndex === colIndex;
          const isStacked = col.type === 'stacked';
          
          // Width logic only applies to Desktop (flex-row). On mobile (flex-col), it's auto.
          let columnFlexClass = 'flex-1';
          if (isColumnActive) {
            columnFlexClass = isStacked ? 'flex-[1.5]' : 'flex-[4]';
          }

          // Max Width constraints adjusted for 3 main cards
          const maxWidthClass = isStacked 
            ? 'max-w-none md:max-w-[45vh] lg:md:max-w-[52vh]' 
            : 'max-w-none md:max-w-[60vh] lg:max-w-[70vh]';

          return (
            <div 
              key={colIndex} 
              className={`${columnFlexClass} flex flex-col transition-all duration-500 ease-out border-b md:border-b-0 md:border-r border-white/10 last:border-b-0 md:last:border-r-0 ${maxWidthClass} w-full`}
            >
              {col.items.map((item, itemIndex) => {
                ...
                const animationDelay = `${(item.id - 1) * 100}ms`;
                ...
                return (
                  <MenuCard
                    ...
                    className={`
                        ${isStacked && itemIndex === 0 ? 'border-b md:border-b border-white/10' : ''} 
                        ${!isStacked && itemIndex === 0 ? 'mb-4 md:mb-0' : ''}
                    `}
